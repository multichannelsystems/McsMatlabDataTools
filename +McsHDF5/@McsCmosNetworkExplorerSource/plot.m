...
% Plot the contents of a McsCmosNetworkExplorerSource object.
%
...
% INPUT:
%
%   staObject       -   A McsCmosNetworkExplorerSource object
%
...
% (c) 2018 by Multi Channel Systems MCS GmbH
    ...
    set(visualization.localizationFig, ...
        ...
        'Tag','HeatMap', ...
        'WindowButtonDownFcn', @McsHDF5.McsCmosNetworkExplorerSource.mouseHandlerHeatMap);
    ...
    McsHDF5.McsCmosNetworkExplorerSource.plotHeatMap(visualization.localizationFig, coordinates, UnitActivity, sensorDimension);
    ...
    if ~isDefaultSTAs
        for unit=1:length(cfg.sta)
            data.visualization.(['ID' num2str(IDs(unit))]).singleUnit       = McsHDF5.McsCmosNetworkExplorerSource.plotSingleSensor(gcf,cfg.sta(unit), McsHDF5.ID2coordinates(IDs(cfg.sta(unit)), sensorDimension(1), sensorDimension(2)));
        end
    end
