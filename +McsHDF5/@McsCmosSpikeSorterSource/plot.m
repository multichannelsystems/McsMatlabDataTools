...
%%  CREATE CUSTOM GRIDLINES
    AX1     = axes('OuterPosition', AX1_outerPosition);            
...
%     % SET AXES PROPERTIES
     ...
     set(AX1, 'xtick', 0:10:sensorXDimension, ...
...
%%  CREATE AXES, MARK SITES & SET UP ACTIVITY MATRIX
    % plot rectangles
    AX2 = axes('OuterPosition',AX2_outerPosition,'Ydir','reverse','Visible','off'); %for HeatMap rectangles
    AX5 = axes('OuterPosition',AX5_outerPosition,'Ydir','reverse','Visible','off'); %for location markers
    markers = 'none';
    ...
    c                       = colorbar('Location','manual',...
                                ...
                                'Visible',visible,...
                                'Tag','colbarSpikeCount');
    ticks                   = get(c,'TickLabels');
    ticks                   = round(cellfun(@str2num,ticks)*max(spikeCount),-2);
    ticks                   = arrayfun(@num2str,ticks,'UniformOutput',false);
    set(c,'TickLabels',ticks);
    c.Label.String          = 'Spike Count';
    
    data.colbarSpikeCount   = c;
    data.markers            = AX5;
    data.activity           = AX2;
    
% plot labels
    AX3 = axes('OuterPosition',AX2_outerPosition,'Ydir','reverse','Visible','off');
...
%
%%  CREATE STA IMAGE
    AX4 = axes('OuterPosition',AX4_outerPosition,'Visible','off');
    visible = 'off';
    ...
    hImage          = imshow(imresize(image,[1024 1024]),'Colormap',jet);
